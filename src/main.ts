...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    .setTitle('Nexsoll Admin API')
    .setDescription(
      'Public project read APIs + contact form + Admin Secret protected write APIs',
    )
    .setVersion('1.0')
    .addApiKey(
      {
        ...
        name: 'x-admin-secret',
        in: 'header',
        description: 'Admin secret for protected project/lead write operations',
      },
      'admin-secret',
    )
    .build();
  ...
  logger.log(`Nexsoll Admin API running on 0.0.0.0:${port}`);
